...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    // transparent background vector (alpha 0)
    ...
    /*** MOUSE (patched to listen on window so it works under content) ***/
    class MouseClass {
      mouseMoved = false;
      ...
      init(container: HTMLElement) {
        this.container = container;
        // ✅ attach motion listeners to window so they fire even when canvas is behind content
        window.addEventListener("mousemove", this._onMouseMove);
        window.addEventListener("touchstart", this._onTouchStart, { passive: true });
        window.addEventListener("touchmove", this._onTouchMove, { passive: true });
        window.addEventListener("touchend", this._onTouchEnd);
        // keep hover state per container (optional)
    ...
    class Divergence extends ShaderPass {
      ...
      update(...args: any[]) {
        ...
        if (this.uniforms && vel) {
          (this.uniforms as any).velocity.value = vel.texture;
        }
        super.update();
      }
    }
    ...
    class Poisson extends ShaderPass {
      ...
      update(...args: any[]) {
        ...
        for (let i = 0; i < iter; i++) {
          ...
          if (this.uniforms) (this.uniforms as any).pressure.value = p_in.texture;
    ...
    class Pressure extends ShaderPass {
      ...
      update(...args: any[]) {
        ...
        if (this.uniforms && vel && pressure) {
          (this.uniforms as any).velocity.value = vel.texture;
          (this.uniforms as any).pressure.value = pressure.texture;
        }
        super.update();
      }
    }
